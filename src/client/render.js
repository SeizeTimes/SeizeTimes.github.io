const hat = new Image();
hat.src = 'https://cdn.discordapp.com/attachments/937874787202637884/938122180443639808/-export.png'
...
window.render = () => {
    ...
    if (cprogress > 0 && players[selfId]?.clones?.length > 0) {
        ...
        leftCanvas.width =
            canvas.width - canvas.width * ((1 - 0.7) * cprogress); //0.6;
        ...
        ctx.drawImage(
            ...
            50 * cprogress,
            50 * cprogress,
            leftCanvas.width - 100 * cprogress,
            leftCanvas.height - 100 * cprogress
        );
        ctx.drawImage(
            rightCanvas,
            leftCanvas.width,
            50 * cprogress,
            rightCanvas.width - 50 * cprogress,
            rightCanvas.height - 100 * cprogress
        );
...
function drawArrows() {
    for (const arrowId of Object.keys(arrows)) {
        ...
        const pos = offset(x, y);
        if (
            players[selfId]?.characterName === 'Beyond' &&
            players[selfId]?.hasDrone //&&
            // arrows[arrowId].parent !== selfId
        ) {
            const a = offset(future.x, future.y);
            ctx.strokeStyle = 'black';
            ctx.lineWidth = 4;
            ctx.beginPath();
            ctx.lineTo(pos.x, pos.y);
            ctx.lineTo(
                a.x - Math.cos(futureAngle) * 0,
                a.y - Math.sin(futureAngle) * 0
            );
            ctx.stroke();
        }
        ...
        if (
            fake &&
            players[selfId]?.characterName === 'Xerox' &&
            players[selfId].clones &&
            players[selfId].clones.length > 0
        ) {
            let trans = false;
        ...
        if (
            players[arrows[arrowId].parent]?.characterName === 'Vice' &&
            life > 3 &&
            ...
            players[arrows[arrowId].parent]?.characterName === 'Mince' &&
            players[arrows[arrowId].parent]?.abilityCd <= 0
        ) {
            ctx.strokeStyle = 'black';
            ctx.lineWidth = 4;
        ...
        if (
            ...
            const b = offset(future.x, future.y);
            ctx.translate(b.x, b.y);
            ctx.rotate(futureAngle);
            ctx.fillStyle = 'black';
            ctx.font = '50px Inter';
            ctx.fillText('>', -10, 0);
            ctx.rotate(-futureAngle);
...
function renderPlayerEntity(player, playerId, isClone) {
    const maxDistToCamera = 1000;
    if (
        Math.abs(player.pos.x - camera.x) > maxDistToCamera ||
    ...
    ctx.fillStyle = ServerCharacter[player.characterName].Color;
    if (
        player.timer > 0 ||
        (player.characterName === 'Scry' && !player.showAim)
    ) {
        ctx.fillStyle = ServerCharacter[player.characterName].ArrowCdColor;
    }
    ...
    if (
        ...
        //     player.radius + me.droneViewRadius
        // ) {
    ...
    if (player.characterName === 'Stac') {
        ...
        if (
            playerId === selfId &&
            player.arrowing > 0 &&
            player.canCreatePoint
        ) {
            ctx.fillStyle = '#004f4f';
            ctx.globalAlpha = 0.5;
            const preview = offset(
                player.pos.x +
                    Math.cos(player.interpAngle) * (player.radius + 250),
                player.pos.y +
                    Math.sin(player.interpAngle) * (player.radius + 250)
            );
            ctx.beginPath();
    ...
    if (
        ...
        for (const pi of Object.keys(players)) {
            ...
            if (
                other.timer > 0 ||
                (other.characterName === 'Scry' && !other.showAim)
            ) {
                continue;
            }
        ...
        if (otherId != null) {
            ...
            const dest = {
                x:
                    other.pos.x +
                    Math.cos(player.interpAngle) *
                        ((player.arrowing / 3) * 200),
                y:
                    other.pos.y +
                    Math.sin(player.interpAngle) *
                        ((player.arrowing / 3) * 200),
            };
            ctx.globalAlpha = 0.8;
    ...
    if (player.canDash && player.characterName === 'Conquest') {
        ...
        let dashPos = offset(
            player.pos.x +
                Math.cos(player.iDashAngle) * 50 +
                Math.cos(player.iDashAngle) * force * 300,
            player.pos.y +
                Math.sin(player.iDashAngle) * 50 +
                Math.sin(player.iDashAngle) * force * 300
        );
    ...
    if (
        player.arrowing > 0 &&
        (player.characterName !== 'Scry' ||
            (player.characterName === 'Scry' &&
                (playerId === selfId || player.showAim)))
    ) {
        let ga =
            player.characterName === 'Scry' &&
            playerId === selfId &&
        ...
        // bow strands
        ctx.lineTo(
            Math.cos(1.25 * Math.PI) * 60,
            Math.sin(1.25 * Math.PI) * 60
        );
        ctx.lineTo(-5, -30 + player.arrowing * 25);
        ctx.lineTo(5, -30 + player.arrowing * 25);
        ctx.lineTo(
            Math.cos(1.75 * Math.PI) * 60,
            Math.sin(1.75 * Math.PI) * 60
        );
        ctx.stroke();
        ...
        ctx.strokeStyle = ServerCharacter[player.characterName].Color;
        if (
            player.characterName === 'Scry' &&
            playerId === selfId &&
            player.canFakeArrow
...
function drawRaycasting() {
    ...
    points = Ray.getPoints(
        new Vec(camera.x, camera.y),
        uniqueRayPoints,
        rayLines,
        -2 + players[selfId]?.radius || 0
    );
    // 	rayCalcT++;
...
function drawOverlay() {
    ...
    if (window.teamMode) {
        ctx.fillStyle = ServerCharacter[players[selfId]?.characterName]?.Color;
...
function drawMinimap() {
    if (players[selfId] != null && !players[selfId].dying && !intermission) {
        ...
        for (const { x, y, width, height, type } of obstacles) {
            ...
            ctx.fillRect(
                (x / arena.width) * mwidth,
                canvas.height - mheight + (y / arena.height) * mheight,
                (width / arena.width) * mwidth,
                (height / arena.height) * mheight
            );
            ctx.globalAlpha = 1;
        }
        ...
        for (const playerId of Object.keys(players)) {
            ...
            if (player.dying) {
                ...
                ctx.fillText(
                    'X',
                    (player.pos.x / arena.width) * mwidth,
                    canvas.height -
                        mheight +
                        (player.pos.y / arena.height) * mheight
                );
            } else {
                if (
                    playerId === selfId &&
                    player.characterName === 'Stac' &&
                    player.point_x != null &&
                    player.point_y != null
                ) {
                    ...
                    ctx.beginPath();
                    ctx.arc(
                        (player.point_x / arena.width) * mwidth,
                        canvas.height -
                            mheight +
                            (player.point_y / arena.height) * mheight,
                        (30 / arena.width) * mwidth,
                        0,
                        Math.PI * 2
                    );
                    ctx.fill();
                ...
                ctx.arc(
                    ...
                    canvas.height -
                        mheight +
                        (player.pos.y / arena.height) * mheight,
                    (player.radius / arena.width) * mwidth,
                    0,
                    Math.PI * 2
                );
                ctx.fill();
                if (player.clones.length > 0) {
                    player.clones.forEach((clone) => {
                        ctx.beginPath();
                        ctx.arc(
                            (clone.pos.x / arena.width) * mwidth,
                            canvas.height -
                                mheight +
                                (clone.pos.y / arena.height) * mheight,
                            (clone.radius / arena.width) * mwidth,
                            0,
                            Math.PI * 2
                        );
                        ctx.fill();
                ...
                if (player.hasDrone && playerId === selfId) {
                    ctx.strokeStyle = '#ff4326';
                    ctx.fillStyle = '#ff4326';
                    ctx.lineWidth = 4;
                    ctx.strokeRect(
                        2,
                        canvas.height - mheight + 2,
                        mwidth - 4,
                        mheight - 4
                    );
                    // ctx.beginPath();
                    // ctx.arc(
                    //     (player.dronePos.x / arena.width) * mwidth,
                    //     canvas.height -
                    //         mheight +
                    //         (player.dronePos.y / arena.height) * mheight,
                    //     (player.droneViewRadius / arena.width) * mwidth,
                    //     0,
                    //     Math.PI * 2
                    // // );
                    // ctx.globalAlpha = 0.1;
                    ...
                    ctx.arc(
                        ...
                        canvas.height -
                            mheight +
                            (player.dronePos.y / arena.height) * mheight,
                        ((player.droneRadius + 25) / arena.width) * mwidth,
                        0,
                        Math.PI * 2
                    );
                    ctx.fill();
                    for (const arrowId of Object.keys(arrows)) {
                        ...
                        if (
                            Math.sqrt(distX * distX + distY * distY) <
                            30 + player.droneViewRadius
                        ) {
                            ctx.fillStyle = 'red';
                            ctx.beginPath();
                            ctx.arc(
                                (arrow.pos.x / arena.width) * mwidth,
                                canvas.height -
                                    mheight +
                                    (arrow.pos.y / arena.height) * mheight,
                                (30 / arena.width) * mwidth,
                                0,
                                Math.PI * 2
                            );
...
function drawDebugText() {
    ...
    if (window.debug) {
        ctx.fillText(
            ...
            } | Download: ${stateMessageDisplay} msg/s (${(
                byteDisplay / 1000
            ).toFixed(1)}kb/s | Upload: ${(uploadByteDisplay / 1000).toFixed(
                1
            )}kb/s | ${inputMessageDisplay} msg/s (inputs) | Ping: ${ping}ms | Spacing:[${lowest(
                spacings
            ).toFixed(1)}, ${spacing.toFixed(1)}, ${highest(spacings).toFixed(
                1
            )}]ms | ServerSpacing: [${serverSpacing[0]}, ${serverSpacing[1]}, ${
                serverSpacing[2]
            }] | Angle: ${players[selfId]?.angle.toFixed(1)}`,
            200,
            800
        );
        ctx.fillText(
            `Extralag: ${extraLag} | Interpolate: ${window._interpolate
                .toString()
                .toUpperCase()} | Input Delay: ${Math.ceil(
                (ping * 2) / (1000 / 60)
            )} frames | Arrows: ${
                Object.keys(arrows).length
            } | ServerTickTime: ${serverTickMs}ms | ServerFrameTime: ${Math.round(
                serverTickMs / 60
            )}ms | ${window.fps}fps | Raycast end points: ${
                window.rayPointsLen
            }`,
            200,
            825
...
function drawPanel() {
    ...
    ctx.fillText(
        `[R] Auto Respawn: ${window.autoRespawn ? 'On' : 'Off'}`,
        canvas.width - 160,
        canvas.height - 15
    );
    ...
    ctx.fillText(
        `[P] ${window.movementMode === 'wasd' ? 'WASD' : 'ULDR'}`,
        canvas.width - 355,
        canvas.height - 15
    );
...
function drawLeaderboard() {
    if (tab || intermission) {
        ...
        for (const id of sortedPlayers) {
            ...
            ctx.fillText(
                `${strScore}`,
                990 - ctx.measureText(`${strScore}`).width,
                top + 25 + index * 25
            );
            index++;
            ctx.fillStyle = '#ff0000';
            ...
            ctx.fillText(
                `${
                    iExist && !dead && players[selfId].dev ? `[${id}] ` : ''
                }${name}`,
                640,
                top + index * 25
            );
...
function drawTimer() {
    ...
    ctx.font = `25px ${font}`;
    ctx.fillText(
        `${window.disconnected ? 'DISCONNECTED' : convert(roundTime)}`,
        800,
        20
    );
}
    ...
    if (players[selfId] && !intermission) {
        ...
        actx.arc(
            ...
            Math.PI *
                2 *
                Math.max(
                    players[selfId].abilityCooldown / players[selfId].maxCd,
                    0
                ),
            0
        );
        ...
        actx.globalAlpha = 1;
        if (
            players[selfId].abilityCd <= 0 &&
            players[selfId].characterName === 'Klaydo'
        ) {
            ...
            actx.arc(
                ...
                -Math.PI *
                    2 *
                    Math.max(
                        players[selfId].timeSpentFreezing /
                            players[selfId].timeFreezeLimit,
                        0
                    ),
                0
            );
